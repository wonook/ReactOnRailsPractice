import { compose, createStore, applyMiddleware, combineReducers } from 'redux';

import { routerReducer } from 'react-router-redux';

// See https://github.com/gaearon/redux-thunk and http://redux.js.org/docs/advanced/AsyncActions.html
// This is not actually used for this simple example, but you'd probably want to use this once your app has
// asynchronous actions.
import middleware from 'redux-thunk';

import reducers from '../reducers';
import { initialStates } from '../reducers';

export default props => {
  // This is how we get initial props Rails into redux.
  const { name, todos, filter } = props;
  const { $$helloWorldState, $$todoState, $$visibilityState } = initialStates;

  // Redux expects to initialize the store using an Object, not an Immutable.Map
  const initialState = {
    $$helloWorldStore: $$helloWorldState.merge({
      name,
    }),
    $$todoStore: $$todoState.merge({
      todos,
    }),
    $$visibilityStore: $$visibilityState.merge({
      filter,
    })
  };

  const reducer = combineReducers({
    ...reducers,
    routing: routerReducer,
  });

  return applyMiddleware(middleware)(createStore)(reducer, initialState);
};
